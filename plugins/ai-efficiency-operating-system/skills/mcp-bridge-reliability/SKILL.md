...
1. Record client, server, device, account, endpoint, protocol/spec version, process identity, and any application-level state or affinity handle that must survive across calls.
2. Verify `configured → process alive → transport reachable → protocol-version/client metadata valid → authorized → capability/tool discovery when needed → representative call → observable target effect`.
...
Fingerprint the actual MCP protocol and SDK on the client/server pair before applying lifecycle assumptions. In MCP `2026-07-28`, the protocol core is stateless: the legacy `initialize`/`initialized` exchange and `Mcp-Session-Id` are retired; each request carries protocol version plus client identity/capabilities, and `server/discover` is optional. Tool/resource/prompt list responses may be cacheable, so reconnect validation must also account for cache freshness. Application state may still exist, but it should be explicit rather than confused with a hidden protocol transport session.
...
Test applicable cases: cold start, normal call, server restart, client restart, network interruption, auth expiration, port collision, duplicate launch, wrong-device/account affinity, stale application handle or cached capability catalog, rapid consecutive calls, and recovery after partial failure.
