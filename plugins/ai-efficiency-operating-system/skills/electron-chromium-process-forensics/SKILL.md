...
A zombie/crash-loop hypothesis is weakened when children have live parents, crash handlers roughly match independent roots, no crash/termination evidence exists, renderer counts are stable, memory/handles plateau, or lag occurs without lifecycle churn.
...
- Treating a single large renderer as proof of a leak.
...
**REQUIRED SUB-SKILL:** use `agent-runtime-forensics` for causal attribution and `agent-observability-slos` when lifecycle events must be correlated with user-visible latency.
