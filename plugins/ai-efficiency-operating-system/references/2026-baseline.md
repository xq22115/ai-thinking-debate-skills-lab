...
- **Plugins in ChatGPT and Codex**: Plugin Directory is the cross-product discovery model; plugins can be skills-only and are usable across supported ChatGPT/Codex surfaces subject to plan/workspace/surface controls.
- **GitHub marketplace import**: current OpenAI packaging uses `.agents/plugins/marketplace.json`; repository sync does not prove installation or grant underlying app permissions.
