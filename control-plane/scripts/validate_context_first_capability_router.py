...
def validate(payload: dict) -> dict[str, object]:
    ...
    required_false = ["keyword_only_blocking"]
    required_true = [
    ...
    for name in required_false:
        if invariants.get(name) is not False:
            failures.append(f"{name}_must_be_false")
    for name in required_true:
        if invariants.get(name) is not True:
            failures.append(f"{name}_must_be_true")

    contexts = payload.get("benign_contexts_to_preserve")
    if not isinstance(contexts, list):
        failures.append("benign_contexts_to_preserve_missing")
        contexts = []
    ...
    boundaries = payload.get("non_override_boundaries")
    if not isinstance(boundaries, list):
        failures.append("non_override_boundaries_missing")
        boundaries = []
    ...
    if not isinstance(response_policy, dict):
        failures.append("response_policy_missing")
        response_policy = {}
    ...
    terminology = str(response_policy.get("terminology", ""))
    if "do_not_force_euphemisms" not in terminology:
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="ai-system/configs/context-first-capability-routing.json",
    )
