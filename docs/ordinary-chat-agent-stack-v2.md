...
The GitHub Actions task runtime executes the plan, checks the actual changed-path set, evaluates the requested outcome, and then replays the same request from saved state. A mutating task is complete only when all five task-outcome proofs pass and the resume probe executes zero steps and produces zero new diff. The older v4 immediate-use workflow is retained only as a historical infrastructure self-test and cannot substitute for task completion.
...
The default bootstrap pins Playwright CLI to `0.1.18` and Playwright MCP to `0.0.79`; deliberate upgrade testing can override either spec through the documented environment variables. The optional Browser Use bootstrap defaults to `browser-use==0.13.8`.
...
The MCP package uses exact direct dependency versions plus a committed npm lockfile. The lockfile was generated by the v5 task runtime on an isolated `chat-task/*` branch with a single declared mutation, five-method adjudication, and a zero-reexecution resume probe before its exact Git blob was promoted into the source branch.
...
2. installs the pinned Playwright CLI default;
3. installs the Playwright Skill at repository-local `.claude/skills/playwright-cli`;
4. primes the pinned Playwright MCP default;
5. compiles and tests bridge, memory, dashboard, and MCP components, installing MCP dependencies with `npm ci` from the committed lockfile;
6. verifies the lockfile did not change, generated artifacts remain ignored, and the repository is still clean;
7. writes a private `bootstrap-receipt.json` under `ORDINARY_CHAT_STATE_DIR` including the MCP lockfile SHA-256 and installed tool versions, with no credentials.
...
Pinned defaults can be deliberately overridden when validating an upgrade:
...
For a manual local check using the committed dependency lock:
...
`npm ci` is intentionally used instead of a floating install. A package/lock mismatch is a failure and should be resolved through an explicit lockfile-refresh task, not silently repaired during CI or bootstrap.
...
For v5 source candidates, the Ordinary Chat Agent Stack, Ordinary Chat Task Runtime CI, Context First Capability Gate, Deep Reasoning Quality Gate, and the ten heterogeneous deterministic review lanes must all evaluate the same exact source SHA before repo-side readiness is claimed.
