...
## 13. Semantic dialogue-state routing extension

This section retains PR #50's demand-loaded multi-turn routing rules and composes them with v1.4's Goal Contract, decision-robustness, and distribution-shift gates.

### Additional inputs

When argument history can change which premises are available, also classify:
...
### Gate D escalation rule

After ordinary `semantic-argument-microscope` normalization, load `semantic-argument-microscope/DIALOGUE_STATE.md` **only** when a later conclusion materially depends on whether an earlier proposition was genuinely accepted, merely assumed for testing, left unresolved, withdrawn, evidence-pending, or carried across a changed definition/criterion.
...
Before leaving semantic normalization on a material multi-turn dispute, preserve the smallest useful state:
...
### Dialogue-state escalation/de-escalation

Escalation may be justified when dialogue-state repair leaves materially different surviving commitment interpretations that predict different downstream conclusions.

De-escalate when a local common-ground/definition repair resolves the apparent disagreement and no independent factual/causal dispute remains.

### Message retention

When DIALOGUE_STATE is active, always retain:
...
Compress ceremonial state ledgers when no state transition matters.

### Judge safeguards

For material semantic/debate judgments, combine the v1.4 Goal Contract ranking with the bias controls from `multi-agent-deliberation`: blind labels where possible, score independently before debate, use candidate order swaps when preference claims matter, normalize verbosity, run evidence-only checks when useful, audit strong minorities, and keep judge/session separation explicit.

`VOTE_COUNT != EVIDENCE_WEIGHT`.

`JUDGE_CONSENSUS != GROUND_TRUTH`.

### Semantic termination rules
...
- forcing a definitive verdict when the correct state remains genuinely unresolved.